...
contains
    ...
    subroutine handle_pair_event(i, j, tcur, lparticles, zcur, tstamp, w_fall, &
        ...
        real(dp) :: vrel, p, u, r_keep, r_kill
        ...
        ! Collision probability: geometric cross-section / domain cross-section
        p = pi * (lparticles(i)%radius + lparticles(j)%radius)**2 / A
        if (p > 1.0) p = 1.0
        ...
        ! No coalescence: swap adjacency order (they passed through each other)
